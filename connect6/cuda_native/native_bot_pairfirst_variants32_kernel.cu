#include "native_bot_pairfirst_kernel.cu"

namespace {

using namespace v4_detail;

constexpr int TOP_PAIR_K32 = 32;

// Controlled PairFirst variant: exactly the same all-legal-pairs cheap scorer
// as P128, but only the global TOP32 pairs reach the expensive exact evaluator.
__global__ void tactical_search_pairfirst_p32_kernel(
    ...
    __shared__ int selected_first[TOP_PAIR_K32];
    __shared__ int selected_second[TOP_PAIR_K32];
    __shared__ int64_t selected_scores[TOP_PAIR_K32];
    ...
    for (int i = 0; i + 1 < empty_count; ++i) {
        ...
        if (tid == 0) {
            for (int j = i + 1; j < empty_count; ++j) {
                ...
                if (selected_first[TOP_PAIR_K32 - 1] >= 0 &&
                    !cheap_pair_better(
                        pair_score,
                        first,
                        second,
                        selected_scores[TOP_PAIR_K32 - 1],
                        selected_first[TOP_PAIR_K32 - 1],
                        selected_second[TOP_PAIR_K32 - 1])) {
                    continue;
                }

                for (int pos = 0; pos < TOP_PAIR_K32; ++pos) {
                    if (cheap_pair_better(
                            ...
                            selected_first[pos],
                            selected_second[pos])) {
                        for (int s = TOP_PAIR_K32 - 1; s > pos; --s) {
...
extern "C" cudaError_t launch_tactical_bot_pairfirst_p32_cuda(
    ...
    tactical_search_pairfirst_p32_kernel<<<batch, v4_detail::THREADS, 0, stream>>>(
