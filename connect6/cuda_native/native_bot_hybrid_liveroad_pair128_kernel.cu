...
__global__ void tactical_search_hybrid_liveroad_pair128_kernel(
    ...
    // V2 is not a candidate filter here. It is used only for the common
    // immediate single-stone win shortcut and the degenerate <2-live-cells
    // legality fallback at the very beginning/end of a game.
    ...
    // Pure LiveRoad prefilter: only empty cells from clean six-cell roads with
    // >=2 stones of either side enter the normal two-stone search pool. There
    // is deliberately NO TOP16/V2 floor and no extra safety-net cells.
    ...
    if (tid == 0) {
        ...
        // Only a legality fallback. It is not used when LiveRoad produced a
        // valid pair, so it does not broaden the normal hybrid search space.
    ...
    // PairFirst stage, but only inside the pure LiveRoad pool. Every retained
    // unordered pair is cheap-scored exactly once; global TOP128 is exact.
...
extern "C" cudaError_t launch_tactical_bot_hybrid_liveroad_pair128_cuda(
    ...
    tactical_search_hybrid_liveroad_pair128_kernel<<<batch, v4_detail::THREADS, 0, stream>>>(
