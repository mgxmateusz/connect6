#include "native_bot_v3_kernel.cu"
...
constexpr int PRO_V3_CANDIDATE_K = 16;

// Identical search/evaluator to V3. Only V2 candidate ordering is replaced by
// V2Pro so this is a controlled scorer A/B test.
__global__ void tactical_search_v3_pro_top16_kernel(
    ...
    __shared__ int candidate_actions[PRO_V3_CANDIDATE_K];
    __shared__ int candidate_scores[PRO_V3_CANDIDATE_K];
    ...
    __syncthreads();
    if (tid == 0) {
        select_top_k_serial<PRO_V3_CANDIDATE_K>(
    ...
    #pragma unroll 1
    for (int i = 0; i < PRO_V3_CANDIDATE_K; ++i) {
        const int first = candidate_actions[i];
        if (first < 0) continue;
        #pragma unroll 1
        for (int j = i + 1; j < PRO_V3_CANDIDATE_K; ++j) {
            ...
            const int value = evaluate_state_parallel(
                ...
                &my_overflow, &opp_overflow,
                &my_win, &opp_win, &eval_result);
...
extern "C" cudaError_t launch_tactical_bot_v3_pro_top16_cuda(
    ...
    tactical_search_v3_pro_top16_kernel<<<batch, v4_detail::THREADS, 0, stream>>>(
