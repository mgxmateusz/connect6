#include "native_bot_hybrid_liveroad_pair128_kernel.cu"

namespace {

using namespace v4_detail;

// Same pure LiveRoad pool and same cheap pair-aware score as Hybrid128.
// Only the number of finalists passed to the expensive exact evaluator changes.
template <int PAIR_K>
__global__ void tactical_search_hybrid_liveroad_pairk_kernel(
    ...
    __shared__ int selected_first[PAIR_K];
    __shared__ int selected_second[PAIR_K];
    __shared__ int64_t selected_scores[PAIR_K];
    ...
    if (tid == 0) {
        ...
        // Same legality-only fallback as Hybrid128. No V2 widening when a
        // valid LiveRoad pair exists.
    ...
    for (int i = 0; i + 1 < candidate_count; ++i) {
        ...
        if (tid == 0) {
            for (int j = i + 1; j < candidate_count; ++j) {
                ...
                if (selected_first[PAIR_K - 1] >= 0 &&
                    !cheap_pair_better(
                        pair_score,
                        first,
                        second,
                        selected_scores[PAIR_K - 1],
                        selected_first[PAIR_K - 1],
                        selected_second[PAIR_K - 1])) {
                    continue;
                }

                for (int pos = 0; pos < PAIR_K; ++pos) {
...
extern "C" cudaError_t launch_tactical_bot_hybrid_liveroad_pair96_cuda(
    ...
    tactical_search_hybrid_liveroad_pairk_kernel<96><<<batch, v4_detail::THREADS, 0, stream>>>(
...
extern "C" cudaError_t launch_tactical_bot_hybrid_liveroad_pair64_cuda(
    ...
    cudaStream_t stream) {
    if (batch <= 0) return cudaSuccess;
    tactical_search_hybrid_liveroad_pairk_kernel<64><<<batch, v4_detail::THREADS, 0, stream>>>(
        boards, current_player, stones_left, pending_second, actions, batch);
    return cudaGetLastError();
}
