...
// Benchmark/reference bot only: when two stones are available it evaluates
// every legal unordered pair C(E,2), where E is the number of empty cells.
// It intentionally uses exactly the same full-state evaluator as V3 so the
// only difference is candidate pruning (TOP16 vs exhaustive search).
__global__ void tactical_search_full_pair_kernel(
    ...
    // Same single-cell ordering/tie-breaker as V3. It does not prune anything.
    ...
    // Preserve V3's immediate single-stone win shortcut. Outside that case,
    // every legal unordered pair is evaluated with the exact same state value.
    if (selected_first >= 0) {
        if (tid == 0) {
    ...
    for (int first = 0; first < CELLS; ++first) {
        if (src[first] != 0) continue;

        #pragma unroll 1
        for (int second = first + 1; second < CELLS; ++second) {
            if (src[second] != 0) continue;
...
extern "C" cudaError_t launch_tactical_bot_full_pair_cuda(
    ...
    tactical_search_full_pair_kernel<<<batch, v4_detail::THREADS, 0, stream>>>(
        boards,
        current_player,
        stones_left,
        pending_second,
