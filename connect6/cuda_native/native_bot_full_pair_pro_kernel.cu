#include "native_bot_full_pair_kernel.cu"
...
// Exhaustive control: evaluates exactly the same every legal unordered pair as
// Full. V2Pro changes only one-stone behavior, immediate-win ordering and the
// tie-break among states with exactly equal full evaluator value.
__global__ void tactical_search_full_pair_pro_kernel(
...
extern "C" cudaError_t launch_tactical_bot_full_pair_pro_cuda(
    ...
    tactical_search_full_pair_pro_kernel<<<batch, v4_detail::THREADS, 0, stream>>>(
