#include "native_bot_pairfirst_variants32_kernel.cu"
...
__device__ __forceinline__ int64_t cheap_pair_score_pro_prior(
    const int8_t* board,
    int first,
    int second,
    int8_t player,
    ...
    // Pair-native tactical classes remain authoritative. The endpoint prior is
    // deliberately scale=1: a ~90M latent-fork endpoint can refine the 120M
    // own-three class, but cannot overpower 30B four / 220B five / 1e15 hard
    // win-defense classes.
...
template <int PRO_PAIR_K>
__global__ void tactical_search_pairfirst_pro_kernel(
    ...
    __shared__ int selected_first[PRO_PAIR_K];
    __shared__ int selected_second[PRO_PAIR_K];
    __shared__ int64_t selected_scores[PRO_PAIR_K];
    ...
    // Exactly the same all-legal-pairs enumeration as PairFirst baseline.
    for (int i = 0; i + 1 < empty_count; ++i) {
        ...
        for (int j = i + 1 + tid; j < empty_count; j += blockDim.x) {
            ...
            row_scores[j] = cheap_pair_score_pro_prior(
                board, first, second, player,
        ...
        if (tid == 0) {
            for (int j = i + 1; j < empty_count; ++j) {
                ...
                if (selected_first[PRO_PAIR_K - 1] >= 0 &&
                    !cheap_pair_better(pair_score, first, second,
                        selected_scores[PRO_PAIR_K - 1],
                        selected_first[PRO_PAIR_K - 1],
                        selected_second[PRO_PAIR_K - 1])) {
                    continue;
                }
                for (int pos = 0; pos < PRO_PAIR_K; ++pos) {
                    if (cheap_pair_better(pair_score, first, second,
                            selected_scores[pos], selected_first[pos], selected_second[pos])) {
                        for (int s = PRO_PAIR_K - 1; s > pos; --s) {
...
extern "C" cudaError_t launch_tactical_bot_pairfirst_pro_p128_cuda(
    ...
    tactical_search_pairfirst_pro_kernel<128><<<batch, v4_detail::THREADS, 0, stream>>>(
...
extern "C" cudaError_t launch_tactical_bot_pairfirst_pro_p32_cuda(
    ...
    int16_t* pending_second, int64_t* actions, int batch, cudaStream_t stream) {
    if (batch <= 0) return cudaSuccess;
    tactical_search_pairfirst_pro_kernel<32><<<batch, v4_detail::THREADS, 0, stream>>>(
        boards, current_player, stones_left, pending_second, actions, batch);
    return cudaGetLastError();
}
