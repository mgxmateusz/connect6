...
constexpr int REPLY_K = 6;

__global__ void tactical_search_v4_top12_replypair6_kernel(
    ...
    __syncthreads();
    if (tid == 0) {
        select_top_k_serial<CANDIDATE_K>(scores, candidate_actions, candidate_scores);
    ...
    // Evaluate all C(12,2)=66 own pairs and keep the best four.
    #pragma unroll 1
    ...
    for (int q = 0; q < KEEP_K; ++q) {
        ...
        // Rank every legal opponent cell with V2, keep TOP6, then evaluate all
        // C(6,2)=15 unordered full two-stone reply pairs.
...
extern "C" cudaError_t launch_tactical_bot_v4_top12_replypair6_cuda(
    ...
    tactical_search_v4_top12_replypair6_kernel<<<batch, v4_detail::THREADS, 0, stream>>>(
