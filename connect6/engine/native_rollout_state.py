...
class NativeRolloutState:
    """Persistent game state; PPO update boundaries never reset these boards."""
...
class NativeRolloutBuffer:
    ...
    def __init__(
        ...
        self.envs = int(envs)
        # Temporary unfinished segments may coexist with all completed samples.
        # They are filtered out after collect, but this reserve avoids waiting for
        # them to finish just to protect buffer capacity.
        ...
        self.boards = torch.empty(
            (self.capacity, BOARD, BOARD), dtype=torch.int8, device=device
        )
        ...
        self.episode_results = torch.empty(
            self.episode_capacity, dtype=torch.int8, device=device
        )
    ...
    def completed_samples(self, gamma: float) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if indices.numel() == 0:
            return indices, torch.empty(
                0, dtype=torch.float32, device=self.boards.device
            )
...
def _validate_current_model(model: torch.nn.Module) -> torch.nn.Module:
    ...
    if kernels != EXPECTED_KERNELS or channels != EXPECTED_CHANNELS:
        raise RuntimeError(
            ...
            f"{EXPECTED_KERNELS}, channels={EXPECTED_CHANNELS}; "
            f"otrzymano {kernels}, {channels}."
        )
    if int(getattr(model, "input_channels", 0)) != 4:
        raise RuntimeError("Native rollout wymaga 4 kanałów wejściowych")
    if len(getattr(model, "norms", ())) != 8:
        raise RuntimeError("Native rollout wymaga GroupNorm po wszystkich 8 convach")
    return model
...
def _validate_historical_checkpoint(cp: HistoricalCheckpoint) -> None:
    ...
    if int(cfg.get("architecture_version", 0)) != 5:
        raise RuntimeError(
            f"{cp.path.name}: native rollout przyjmuje tylko architecture_version=5"
        )
    ...
    if kernels != EXPECTED_KERNELS or channels != EXPECTED_CHANNELS:
        raise RuntimeError(f"{cp.path.name}: niezgodna architektura CNN V5")
...
def pack_rollout_models(
    ...
    *,
    ...
    norm_weights = [
        torch.empty((num_models, out_c), dtype=dtype, device=device)
        for out_c in EXPECTED_CHANNELS
    ]
    norm_biases = [
        torch.empty((num_models, out_c), dtype=dtype, device=device)
        for out_c in EXPECTED_CHANNELS
    ...
    for layer in range(8):
        current_w = base.convs[layer].weight.detach().reshape(
            1, EXPECTED_CHANNELS[layer], -1
        ).to(device=device, dtype=dtype)
        _copy_padded_conv(conv_weights[layer], slice(0, 1), current_w)
        norm_weights[layer][0].copy_(
            base.norms[layer].weight.detach().to(device=device, dtype=dtype)
        )
        norm_biases[layer][0].copy_(
            base.norms[layer].bias.detach().to(device=device, dtype=dtype)
        )

    policy_weight[0].copy_(
        base.policy_output.weight.detach().reshape(-1).to(device=device, dtype=dtype)
    )
    policy_bias[0].copy_(
        base.policy_output.bias.detach().reshape(()).to(device=device, dtype=dtype)
    )
    value_weight[0].copy_(
        base.value_output.weight.detach().reshape(-1).to(device=device, dtype=dtype)
    )
    value_bias[0].copy_(
        base.value_output.bias.detach().reshape(()).to(device=device, dtype=dtype)
    )
