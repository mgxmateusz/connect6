...
def test_gamma_discount_is_per_stone_and_terminal_action_has_distance_zero():
    ...
    buffer = CompleteGameBuffer(target_completed_positions=4, envs=1, board_size=3, device=device)
    ...
    indices, returns = buffer.completed_samples(episode_results, episode_terminal_moves, gamma=0.5)
    assert indices.tolist() == [0, 1]
    assert returns.tolist() == pytest.approx([0.5**3, 0.5])


def test_temperature_uses_one_based_update_numbers():
    cfg = {"temperature_start": 1.0, "temperature_end": 0.5, "temperature_decay_updates": 10}
...
def test_historical_fraction_is_split_evenly_and_colors_are_balanced():
    mask, opponent_ids, colors = _historical_layout(
        num_envs=100, fraction=0.25, historical_models=4, device=torch.device("cpu")
    )
...
def test_historical_layout_with_fewer_models_never_drops_tables():
    mask, opponent_ids, colors = _historical_layout(
        num_envs=16, fraction=0.5, historical_models=3, device=torch.device("cpu")
    )
...
def test_weighted_history_sampling_allows_duplicates_and_biases_recent(monkeypatch):
    ...
    monkeypatch.setattr("connect6.engine.history.random.choice", lambda pool: pool[0])
...
def test_grouped_history_ensemble_matches_individual_v6_models_on_cpu():
...
def test_online_symmetry_board_action_and_inverse_action_are_consistent():
    ...
    for phase in range(8):
        ...
        restored_actions = _inverse_transform_actions(transformed_actions, board_size=3, k=k, flip=flip)
...
def test_random_black_opening_mask_only_selects_fresh_black_first_move():
    ...
    forced_all = _forced_random_opening_mask(move_count, current_player, stones_left, fraction=1.0)
    forced_none = _forced_random_opening_mask(move_count, current_player, stones_left, fraction=0.0)
